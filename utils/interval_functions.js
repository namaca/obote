...
const dbl = require('../clients/mysql.js').connection;



const checkLiveStatus = (output) => new Promise(async(resolve, reject) => {
    const getChannels = await custom.dbQuery(`SELECT * FROM obote_channels`);
    const parseForUrl = getChannels.map(i => `&user_login=${i.channel}`)

    const getAccessToken = await custom.dbQuery(`
        SELECT *
        FROM access_token
        `);
...
async function sendChannelStatus() {
    async function sendQuery(status, name) {
        await custom.dbQuery(`
            UPDATE obote_channels
            SET status="${status}"
